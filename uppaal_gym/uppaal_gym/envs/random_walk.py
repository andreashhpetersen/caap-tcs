...
class RandomWalkEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, unlucky=False):
        ...
        self.observation_space = gym.spaces.Box(
            low=np.array([0., 0.], dtype=self.dtype),
            high=np.array([1.2, 1.2], dtype=self.dtype)
        )
    ...
    def _get_obs(self):
        return np.array([self.x, self.t], dtype=self.dtype)
    ...
    def _step(self, state, action):
        ...
        # apply randomness
        nx -= self.eps if self.unlucky else self._get_random()
        nt -= -self.eps if self.unlucky else self._get_random()
