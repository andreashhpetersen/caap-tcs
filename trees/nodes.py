...
class Node:
    ...
    def __gt__(self, other : float):
        """
        Compares `other` float to `self.bound` and uses `np.isclose` to test
        for equality in case of floating point imprecision
        """
        return self.bound > other and not np.isclose(other, self.bound)
    ...
    def get_for_region(self, min_state, max_state, collect):
        """
        Get all leaves in the region `[min_state, max_state]` and add them to
        the list `collect`
        """
        if min_state[self.var_id] < self.bound:
            if self.low.is_leaf:
                collect.append(self.low)
            else:
                self.low.get_for_region(min_state, max_state, collect)

        if max_state[self.var_id] > self.bound:
            if self.high.is_leaf:
                collect.append(self.high)
            else:
                self.high.get_for_region(min_state, max_state, collect)
    ...
    @classmethod
    def make_node_from_leaf(cls, leaf, variables, sentinel=None):
        """
...
class Leaf:
    ...
    def split(self, v, bound, state):
        ...
        v_id = state.var2id[v] if isinstance(v, str) else v
