...
for model in models:
    ...
    for _ in range(k):
        ...
        aggr_results.append(cont_results)
    aggr_results = np.array(aggr_results).T.mean(axis=1)
    all_control.append(aggr_results)
...
# store results
shield_df.to_csv(f'{RESULTS_DIR}shield_res.csv')
control_df.to_csv(f'{RESULTS_DIR}control_res.csv')
combined_df.to_csv(f'{RESULTS_DIR}combined_res.csv')
